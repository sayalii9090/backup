. C:\Database\scripts\fullBackupvariable.ps1
...
Add-Content -Path $localLogPath -Value "Backup started for DB $databaseName" 

$backupFile = $backupDirectory + $databaseName + "_" + $date + ".bak"
$awsfile = $databaseName + "_" + $date + ".bak"

sqlcmd -S $instance -Q "BACKUP DATABASE [$databaseName] TO DISK='$backupFile'" -U $username -P $password >> $localLogPath
...
Add-Content -Path $localLogPath -Value "*********************************************************** `n" 
Add-Content -Path $localLogPath -Value "`n$databaseName backup upload to s3 initiated on $date `n" 
