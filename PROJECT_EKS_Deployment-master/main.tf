...
terraform {
  backend "s3" {
    bucket = "eks-projectt-1"
    # key = "Tejas/terraform/stateLock/terraform.tfstate"
    encrypt = true
    # region = "ap-south-1"
    # dynamodb_table = "Eks-project"
  }
}
...
resource "aws_iam_role" "cluster_role" {
  name               = "${var.Cluster_role_id}-Cluster-Role"
...
resource "aws_iam_role" "node" {
  name = "${var.Cluster_role_id}-worker-role"
